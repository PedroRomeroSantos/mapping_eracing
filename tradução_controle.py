...
def callback(msg):
    # Verificamos se o array tem os 3 valores [steering, throttle, brake]
    if len(msg.data) >= 2:
        cmd = ControlCommand()
        # Header é importante em algumas versões do FSDS
        cmd.header.stamp = rospy.Time.now()
        
        # Mapeamento correto dos índices
        cmd.steering = float(msg.data[0])
        cmd.throttle = float(msg.data[1])
        cmd.brake = 0.0 # Force o freio em zero absoluto
        
        pub.publish(cmd)
        # Log para conferir no terminal do ROS 1
