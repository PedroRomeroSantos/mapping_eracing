...
class PathPlanner(Node):
    ...
    def conesCallback(self, msg):
        """Recebe [u, v, Z, ID], calcula midpoints LOCAIS para o controle e GLOBAIS para o gráfico."""
        ...
        for cone_raw in data_raw:
            ...
            # 1. Pixel -> Coordenadas Locais (Sempre relativas ao carro)
            ...
            # Guardamos os dados LOCAIS para o processamento de trajetória
            working_data_local.append([X_w, Z_w, side])
            
        working_data_local = np.array(working_data_local)
        if len(working_data_local) == 0: return

        # 2. SEPARAÇÃO E PAREAMENTO (Ainda em coordenadas locais)
        left_local = working_data_local[working_data_local[:, 2] == LADO_ESQUERDO]
        right_local = working_data_local[working_data_local[:, 2] == LADO_DIREITO]
        
        # O pareamento é feito no frame do carro
        midpoints_local, paired_cones_local = self._pair_and_calculate(left_local, right_local)

        if len(midpoints_local) > 0:
            # --- MUDANÇA ESSENCIAL: PUBLICAÇÃO LOCAL ---
            # Enviamos X e Z relativos ao carro para o PID não saturar
            self._publish_waypoints(midpoints_local)

            # --- MUDANÇA PARA O PLOT: CONVERSÃO GLOBAL ---
            # Convertemos apenas para visualização no Matplotlib
            midpoints_global = midpoints_local.copy()
            midpoints_global[:, 0] += car_x_atual
            midpoints_global[:, 1] += car_z_atual
            
            cones_global = paired_cones_local.copy()
            cones_global[:, 0] += car_x_atual
            cones_global[:, 1] += car_z_atual

            with self.lock:
                 self.current_paired_cones = cones_global
                 self.current_midpoints = midpoints_global
    ...
    def _publish_waypoints(self, waypoints):
        """Publica a lista de waypoints em COORDENADAS LOCAIS."""
        msg = Float32MultiArray()
        # Ordenamos por Z (profundidade) para o PID focar no ponto mais próximo
        waypoints_sorted = waypoints[np.argsort(waypoints[:, 1])]
        
        # O dado enviado será [X_local1, Z_local1, X_local2, Z_local2...]
